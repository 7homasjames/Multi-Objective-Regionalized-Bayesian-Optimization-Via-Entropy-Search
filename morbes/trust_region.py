...
class TrustRegion(ABC, Module):
    ...
    def _update_training_data(self, X_all: Tensor, Y_all: Tensor) -> None:
        ...
        #print("x=+",self.X_center)
        #exit()
        #print(self.X_center_normalized)
        #exit()
...
class HypervolumeTrustRegion(TrustRegion):
    ...
    def _update_center_and_best_points(
        ...
        if X_center is not None:
            ...
            x = torch.tensor(pareto_X_better_than_ref,dtype=torch.double).clone().detach()
            y = torch.tensor(pareto_Y_better_than_ref,dtype=torch.double).clone().detach()
            #
            print(y.shape)
            print(x.shape)
            ...
            for i in range(len(y[1])):
                reshaped_objective = y[:, i].reshape(-1, 1)
                reshaped_objectives.append(reshaped_objective)

            y1 = reshaped_objectives[0]
            y2 = reshaped_objectives[1]
            y3 = reshaped_objectives[2]
            y4 = reshaped_objectives[3]
            ...
            K = len(pareto_Y_better_than_ref[0])

            a = equation(K, Maxmodel(x,y1))
            b = equation(K, Maxmodel(x,y2))
            c = equation(K, Maxmodel(x,y3))
            d = equation(K, Maxmodel(x,y4))

            

            Max_entropy=torch.tensor([float(x) + float(y) + float(z) + float (w)  for x, y, z , w in zip(a, b, c, d)])
            #Max_entropy=torch.tensor(a)
            #print("Entropy:",Max_entropy)
            

            #print("Results:", sum)
            #center_index_demo = sum.index(max(Max_entropy))
            ...
            if eligible_mask.any():
                ...
                #take Maximum Entropy among eligible points
                base_idx = Max_entropy[eligible_indices].argmax()
                idx = eligible_indices[base_idx].item()
            elif not_taken_mask.any():
                #if all pareto points are taken in the tr, take the best available point
                not_taken_indices = not_taken_mask.nonzero().view(-1)
                base_idx = Max_entropy[not_taken_indices].argmax()
                idx = not_taken_indices[base_idx].item()
            else:
                # take point with Max Entropy
                idx = Max_entropy.argmax().item()
            self.X_center = pareto_X_better_than_ref[idx : idx + 1]
            self.Y_center = pareto_Y_better_than_ref[idx : idx + 1]
            #print(pareto_X_better_than_ref)
            #print(pareto_Y_better_than_ref)
            #trusted_center_X=[]
            #trusted_center_Y=[]

            trusted_center_X.append(self.X_center)
            trusted_center_Y.append(self.Y_center)
            #print("Trusted_center;",trusted_center_Y)
            #exit
...
def equation(K, x_values):
    results = [ ]
    for i in range(len(x_values)):
        # First term: K(1 + ln(2))
        first_term =  (K*(1 + math.log(2)))/2
        

        second_term = x_values[i]
...
def Maxmodel(x,y):
    ...
    for i in range(len(y)):
        x_train = torch.cat([x[:i], x[i+1:]], dim=0).double()
        #y_train = torch.cat((y[:i], y[i+1:]), dim=0).double()
        y_train= torch.cat((y[:i], y[i+1:]), dim=0).float()


        #x_test = torch.tensor(pareto_X_better_than_ref[i])
        ...
        #model = KroneckerMultiTaskGP(x_train, y_train).double()
        #likelihood = model.likelihood.double()
        ...
        for epoch in range(training_iter):
            ...
            #print('Sample: {}, Epoch: {}, Loss1: {}, Loss2 {}'.format(i, epoch, loss1, loss2))
            #optimizer.zero_grad()
            ...
            #print('Sample: {}, Epoch: {}, Loss1: {},'.format(i, epoch, loss,))
        #model1_vars.append(output.variance.mean())

        model_vars.append(output.variance.mean())
        
        print()
        new_value=[]
    
    for variance in model_vars:
        equate = torch.log(torch.sqrt(torch.abs(variance)))
        new_value.append(equate)
