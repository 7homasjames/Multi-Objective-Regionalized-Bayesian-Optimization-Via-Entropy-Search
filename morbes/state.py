...
class TRBOState(Module):
    ...
    def _compute_hv_scalarizations(self, Y: Tensor, hv_weights: Tensor) -> Tensor:
        ...
        """
        epsilon = 0.1
        utopia_values = self.ref_point - epsilon
        weighted_sum = (weights * Y).sum(dim=-1)
        return (
            (((utopia_values).clamp_min(0).unsqueeze(-3) / hv_weights) + weighted_sum)
            .amin(dim=-1)
            .pow(Y.shape[-1])
            .amax(dim=-1)
        )
