...
def run_one_replication(
    ...
    if evalfn == "C2DTLZ2":
        ...
        problem = DTLZ7(negate=False, dim = dim)
        bounds = problem.bounds.to(**tkwargs)

        def f(X):
            return torch.cat([problem(X), problem.evaluate_true(X)],dim= -1)
    ...
    tr_centers_X = [[] for _ in range(n_trust_regions)]
    tr_centers_Y = [[] for _ in range(n_trust_regions)]
    ...
    while trbo_state.n_evals < max_evals:
        ...
        for i, tr in enumerate(trbo_state.trust_regions):
            ...
            tr_centers_X[i].append(tr.X_center.cpu().squeeze().tolist())
            tr_centers_Y[i].append(tr.Y_center.cpu().squeeze().tolist())
        ...
        output = {
            ...
            "tr_sizes": tr_sizes,
            "tr_centers_X": tr_centers_X,
            "tr_centers_Y": tr_centers_Y,
    ...
    save_callback(output)
    print(output)
    print("All_hypervolume:",output["all_hvs"])
